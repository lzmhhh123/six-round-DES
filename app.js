...
const storageEncryption = multer.diskStorage({
...
const uploadEncryption = multer({storage: storageEncryption});

app.post('/api/uploadToEncryption', uploadEncryption.single('file'), (req, res, next) => {
  if (!req.file) {
    res.send(new Error());
  }
  let {keys} = req.body;
...
const storageDecryption = multer.diskStorage({
  destination: './build/decryption/',
  filename: (req, file, cb) => {
    cb(null, 'NeedToDecryption.txt')
  }
})

const uploadDecryption = multer({storage: storageDecryption});

app.post('/api/uploadToDecryption', uploadDecryption.single('file'), (req, res, next) => {
  if (!req.file) {
    res.send(new Error());
  }
  let {keys} = req.body;
  fs.readFile('./build/decryption/NeedToDecryption.txt', (err, data) => {
    if(err) throw err;
    if(data.length === 0) res.send('Decryption completed!')
    let initialVector = new Array(64);
    for (let i = 0; i < 64; ++i) initialVector[i] = 0;
    let clearText64bits = new Array(64);
    let resultBuffer = new Buffer((data.length + 7) / 8 * 8);
    ...
    if (data.length % 8 !== 0) algorithmDES(data.length - 1);
    fs.writeFile('./build/decryption/DecryptedFile.txt', resultBuffer, err => {
      if (err) throw err;
      res.send('Decryption successfully!');
    })
  })
})
